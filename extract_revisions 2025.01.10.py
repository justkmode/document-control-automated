...
def extract_revisions_from_pdfs(folder_path: Path):
    ...
    # Extract fallback date from folder name (e.g. 2025.01.10 -> 2025/01/10)
        ...
        try:
            with fitz.open(pdf_file) as doc:
                for page_num, page in enumerate(doc, start=1):
                    ...
                    # Remove date extraction logic since we no longer need it
                    for drawing in final_drawings:
                        # If the drawing number has already been processed, skip it
...
def extract_revisions_from_pdfs_recursive(folder_path: Path):
    for subfolder in folder_path.rglob('*'):
        if subfolder.is_dir():  # Only process directories
            print(f"Processing folder: {subfolder}")
            extract_revisions_from_pdfs(subfolder)

if __name__ == "__main__":
    folder = Path(r"C:\Users\jukk\OneDrive - COWI\Documents\Revision check\2025.01.10")
    extract_revisions_from_pdfs_recursive(folder)
